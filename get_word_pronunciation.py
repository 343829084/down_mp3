...
def get_page(myword):
    ...
    searchurl=basurl+myword.replace(" ", "+")
...
def get_yingbiao(html_selector):
    ...
    for item in get_yingbiao:
        ...
        if len(it)>0:
            ...
            #下载语音
            down_file=requests.get(ddd[0])
            file_name_1 = str(g_index)+ ".mp3"
            g_index+=1
            file_name_2 = "%d.mp3"%g_index
            g_index+=1
            with open(file_name_1, "wb") as code:
                code.write(down_file.content)
            #复制文件
            open(file_name_2, "wb").write(down_file.content)
...
def get_word(word):
    ...
    return "%s\t%s\t%s\t"%(word,yingbiao,chitiao)
